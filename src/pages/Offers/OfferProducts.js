...
const OfferProducts = () => {
  ...
  const [tenderEndTime, setTenderEndTime] = useState(1);
  ...
  const [rate_time, setRateTime] = useState(0);
  ...
  const [maximunJoiners, setMaximunJoiners] = useState(0);
  ...
  const getproducts = () => {
    axios
      .get("http://localhost:9999/v2/product/getAll?type=admin")
      .then((res) => {
        if (Array.isArray(res.data.message)) {
  ...
  const handleaddoffer = () => {
      ...
      if (i == 0) {
        if (allpossibilities[i].quantity * 1 > 0) {
          varients +=
            ...
            allpossibilities[i].new_price +
            "*" +
            allpossibilities[i].min_price +
            "*" +
            allpossibilities[i].rate_price;
        }
      } else {
        if (allpossibilities[i].quantity * 1 > 0) {
          varients +=
    ...
    axios
      .post("http://localhost:9999/v2/offers/make", data_send)
      .then((res) => {
  ...
  const hanldecopy = () => {
    ...
    axios
      .post("http://localhost:9999/v2/offers/copy", data_send)
      .then((res) => {
        if (res.data.status == 1) {
          toast.success(res.data.message);
          setshowcopy(false);
  ...
  const handlesearchproduct = (txt) => {
    ...
    axios
      .post(
        `http://localhost:9999/v2/product/searchProduct?token=${txt}`,
        data_send
      )
      .then((res) => {
        // console.log(res)
  ...
  const [filterIsTendered, setFilterIsTendered] = useState(false);
  ...
  const handlestopoffer = () => {
    ...
    // console.log(data_send);
    axios
      .post("http://localhost:9999/v2/offers/stop", data_send)
      .then((res) => {
  ...
  useEffect(() => {
    let alldata = [...getoriginlaoffers];
    if (filterIsTendered) {
      setoffers(alldata.filter((item) => item?.isTendered));
    } else {
      setoffers(alldata.filter((item) => !item?.isTendered));
    }
  }, [filterIsTendered]);
          ...
          <div className="childs products_table">
            ...
            <div>
              <label>
                <input
                  type="checkbox"
                  checked={filterIsTendered}
                  onChange={(e) => setFilterIsTendered(e.target.checked)}
                />
                {language === "ar"
                  ? "العروض ضمن المناقصة"
                  : "Offers are Tendered"}
        ...
        <div className="edit_model add_model from_Shipping_div">
          ...
          >
            ...
            <label htmlFor="shippingstatus">
              {language == "ar" ? "ضمن المناقصة" : "Is Tendered"}
            </label>
            <input
              type="checkbox"
              name="shippingstatus"
              id="shippingstatus"
              onChange={(e) => {
                setIsTendered(e.target.checked);
              }}
            />
            {isTendered ? (
              <>
                <label htmlFor="">
                  {language == "ar" ? "المعدل الزمني" : "Time Rate"}
                </label>
                <input
                  name="quantity"
                  onChange={(e) => {
                    setRateTime(e.target.value);
                  }}
                  type="text"
                  placeholder={language == "ar" ? "المعدل الزمني" : "Time Rate"}
                />{" "}
                <label htmlFor="">
                  {language == "ar"
                    ? "أقصي عدد من الملتحقين"
                    : "Maximum Joiners"}
                </label>
                <input
                  name="quantity"
                  onChange={(e) => {
                    setMaximunJoiners(e.target.value);
                  }}
                  type="text"
                  placeholder={
                    language == "ar"
                ...
                />
                <label htmlFor="">
                  {language == "ar" ? "سعر الالتحاق" : "Join Cost"}
                </label>
                <input
                  name="quantity"
                  onChange={(e) => {
                  ...
                  type="text"
                  placeholder={language == "ar" ? "سعر الالتحاق" : "Join Cost"}
                />{" "}
                <label htmlFor="">
                  {language == "ar"
                    ? "مدة انتهاء المناقصة (دقايق)"
                    : "Tender End Time (Minutes)"}
                </label>
                <input
                  name="quantity"
                  onChange={(e) => {
                    setTenderEndTime(e.target.value);
                  }}
                  type="text"
                  placeholder={
                    language == "ar"
              ...
              {possibilities_en.map((item, index) => {
                ...
                return (
                  <div>
                    ...
                    />
                    <label htmlFor="">
                      {language == "ar" ? "السعر النهائي" : "Maximum Price"}
                    </label>
                    <input
                      name="min_price"
                      onChange={(e) => {
                    ...
                    <label htmlFor="">
                      {language == "ar" ? "معدل النقصان " : "Price Rate"}
                    </label>
                    <input
                      name="rate_price"
                      onChange={(e) => {
